...
/*
 **********************************************************
 *** TwinkleLocal-speedy.js: Speedy deletion tagging module
 **********************************************************
...
TwinkleLocal.speedyDeletion = {
	init: function twinkleLocalSpeedyDeletionInit() {
		// Disable on:
		// * special pages
		// * non-existent pages
		...
		Twinkle.addPortletLink(TwinkleLocal.speedyDeletion.callback,
			TwinkleLocal.getLocalValue("speedyDeletion", "tabName"),
			"twlocal-csd",
			/*Morebits.userIsInGroup('sysop') ? "Delete page according to WP:CSD" : */ TwinkleLocal.getLocalValue("speedyDeletion", "tabTooltip"));
	},

	// This function is run when the CSD tab/header link is clicked
	callback: function twinkleLocalSpeedyDeletionCallback() {
	...
	mode: {
		...
		// are we in a mode where multiple criteria can be selected?
	...
	initDialog: function twinklespeedyInitDialog(callbackfunc) {
		var dialog = TwinkleLocal.speedyDeletion.dialog =
			new TwinkleLocal.dialog("speedyDeletion", TwinkleLocal.getPref('speedyWindowWidth'), TwinkleLocal.getPref('speedyWindowHeight') );

		var form = new Morebits.quickForm(callbackfunc,
			(TwinkleLocal.getPref('speedySelectionStyle') === 'radioClick' ? 'change' : null));
		if (TwinkleLocal.userCan('delete')) {
			form.append({
				type: 'checkbox',
				list: [
					{
						...
						event: function( event ) {
							...
							if (cForm.talkpage) {
								...
								cForm.talkpage.checked = !cChecked && TwinkleLocal.getPref('deleteTalkPageOnDelete');
							}
							...
							TwinkleLocal.speedyDeletion.modeChanged(cForm);
