...
* Active on:              Categories whose name starts with "Thể loại:Chờ xóa"
...
Twinkle.deprod = function() {
	if(
		...
		!((/chờ_xóa/i).test(mw.config.get( 'wgPageName' )))
	) {
		return;
	}
	Twinkle.addPortletLink( Twinkle.deprod.callback, "Xóa theo đề nghị", "tw-deprod", "Xóa các trang được đề nghị xóa trong thể loại này");
};
...
callback_deleteRedirects = function( apiobj ) {
	...
	$doc.find("backlinks bl").each(function(){
		...
		page.setEditSummary("[[WP:XN#ĐH1|ĐH1]]: Đổi hướng đến trang không bị xóa “" + apiobj.params.page + "”" + Twinkle.getPref('deletionSummaryAd'));
		page.deletePage();
	});
};
