...
Twinkle.unlink.callbacks = {
	...
	unlinkBacklinks: function twinkleunlinkCallbackUnlinkBacklinks(pageobj) {
		...
		if (params.doImageusage) {
			wikiPage.commentOutImage(mw.config.get('wgTitle'), 'Chú thích hóa');
			text = wikiPage.getText();
			// did we actually make any changes?
			if (text === oldtext) {
				warningString = "sử dụng tập tin";
			} else {
				summaryText = "Chú thích hóa sử dụng tập tin";
				oldtext = text;
			}
		}
		...
		if (params.doBacklinks) {
			...
			// did we actually make any changes?
			if (text === oldtext) {
				warningString = (warningString ? "liên kết đến trang hoặc sử dụng tập tin" : "liên kết đến");
			} else {
				summaryText = (summaryText ? (summaryText + " / ") : "") + "Bỏ liên kết đến";
				oldtext = text;
			}
		}
		...
		if (warningString) {
			// nothing to do!
			pageobj.getStatusElement().error("Không tìm thấy " + warningString + " trên trang.");
