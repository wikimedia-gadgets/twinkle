...
Twinkle.talkback.getNoticeWikitext = function(input) {
	...
	if (input.tbtarget === 'notice') {
		return Morebits.string.safeReplace(Twinkle.talkback.noticeboards[input.noticeboard].text, '$SECTION', input.section);
	}

  if (input.tbtarget === 'mail') {
  ...
  if (input.tbtarget === 'see') {
    return '{{subst:Please see|location=' + input.page + (input.section ? '#' + input.section : '') + '|more=' + input.message + '}}';
  }
