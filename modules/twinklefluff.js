 ...
 */
	...
	Twinkle.fluff.linkBuilder = {
		...
		rollbackLinks: function (vandal, inline, rev, page) {
			...
			var normLink = Twinkle.fluff.linkBuilder.buildLink(
				"SteelBlue",
				"rollback"
			);
			...
			if (!inline) {
				...
				var agfLink = Twinkle.fluff.linkBuilder.buildLink(
					"DarkOliveGreen",
					"rollback (AGF)"
				);
		...
		restoreThisRevisionLink: function (revisionRef, inline) {
			...
			revisionRef =
				typeof revisionRef === "number"
					? revisionRef
					: mw.config.get(revisionRef);
			...
			var revertToRevisionLink = Twinkle.fluff.linkBuilder.buildLink(
				"SaddleBrown",
	...
	Twinkle.fluff.addLinks = {
			...
			if (mw.config.exists("wgRelevantUserName") || isRange) {
				...
				if (
					...
					(mw.config.get("wgUserName") !== username &&
						Twinkle.getPref("showRollbackLinks").indexOf("others") !== -1) ||
					(mw.config.get("wgUserName") === username &&
						Twinkle.getPref("showRollbackLinks").indexOf("mine") !== -1)
				) {
					var $list = $("#mw-content-text").find(
			...
			if (Twinkle.getPref("showRollbackLinks").indexOf("history") !== -1) {
				...
				if (!$("a.mw-firstlink").length) {
					...
					for (var i = 0; i < histList.length; i++) {
						var hasMoreThanOneUser =
							$(histList[i]).find(".mw-userlink").text() !== firstUser;
						if (hasMoreThanOneUser) {
							firstRow.appendChild(
								Twinkle.fluff.linkBuilder.rollbackLinks(firstUser, true)
							);
							break;
						}
					}
		...
		diff: function () {
			...
			var warnFromTalk = function (xtitle) {
				var talkLink = $(
					"#mw-diff-" + xtitle + "2 .mw-usertoollinks a"
				).first();
				if (talkLink.length) {
					var extraParams =
						"vanarticle=" +
	...
	Twinkle.fluff.revert = function revertPage(type, vandal, rev, page) {
		...
		var wikipedia_api = new Morebits.wiki.api(
			"Grabbing data of earlier revisions",
			query,
	...
	Twinkle.fluff.revertToRevision = function revertToRevision(oldrev) {
		...
		var wikipedia_api = new Morebits.wiki.api(
			"Grabbing data of the earlier revision",
			query,
	...
	Twinkle.fluff.callbacks = {
		toRevision: function (apiobj) {
			...
			if (revertToRevID !== apiobj.params.rev) {
				apiobj.statelem.error(
					"The retrieved revision does not match the requested revision. Stopping revert."
				);
				return;
			}

			var optional_summary = prompt(
				"Please specify a reason for the revert:                                ",
				""
			...
			var summary = Twinkle.fluff.formatSummary(
				"Restored revision " + revertToRevID + " by $USER",
			...
			var query = {
				...
				minor:
					Twinkle.getPref("markRevertedPagesAsMinor").indexOf("torev") !== -1
						? true
						: undefined,
				format: "json",
			};
			...
			if (Twinkle.getPref("watchRevertedPages").indexOf("torev") !== -1) {
				...
				if (!watchOrExpiry || watchOrExpiry === "no") {
					...
					watchOrExpiry === "default" ||
					watchOrExpiry === "preferences"
				) {
		...
		main: function (apiobj) {
			...
			if (!page.actions.edit) {
				apiobj.statelem.error(
					"Unable to edit the page, it's probably protected."
				);
				return;
			}
			...
			if (revs.length < 1) {
				statelem.error(
					"We have less than one additional revision, thus impossible to revert."
				);
				return;
			}
			var top = revs[0];
			...
			if (lastrevid < params.revid) {
				Morebits.status.error("Error", [
					"The most recent revision ID received from the server, ",
					Morebits.htmlNode("strong", lastrevid),
					", is less than the ID of the displayed revision. This could indicate that the current revision has been deleted, the server is lagging, or that bad data has been received. Stopping revert.",
				]);
				return;
			}
			...
			if (params.revid !== lastrevid) {
				Morebits.status.warn("Warning", [
					"Latest revision ",
					Morebits.htmlNode("strong", lastrevid),
					" doesn't equal our revision ",
					Morebits.htmlNode("strong", params.revid),
				]);
				// Treat ipv6 users on same 64 block as the same
				if (
					lastuser === params.user ||
					(mw.util.isIPv6Address(params.user) &&
						Morebits.ip.get64(lastuser) === Morebits.ip.get64(params.user))
				) {
					switch (params.type) {
						case "vand":
							var diffUser = lastuser !== params.user;
							Morebits.status.info("Info", [
								"Latest revision was " + (diffUser ? "" : "also ") + "made by ",
								Morebits.htmlNode("strong", userNorm),
								diffUser ? ", which is on the same /64 subnet" : "",
								". As we assume vandalism, we will proceed to revert.",
							]);
							break;
						case "agf":
							Morebits.status.warn("Warning", [
								"Latest revision was made by ",
								Morebits.htmlNode("strong", userNorm),
								". As we assume good faith, we will stop the revert, as the problem might have been fixed.",
							]);
							return;
						default:
							Morebits.status.warn("Notice", [
								"Latest revision was made by ",
								Morebits.htmlNode("strong", userNorm),
								", but we will stop the revert.",
							]);
							return;
					}
				} else if (
					...
					Twinkle.fluff.trustedBots.indexOf(top.user) !== -1 &&
					revs.length > 1 &&
					...
					Morebits.status.info("Info", [
						...
						", a trusted bot, and the revision before was made by our vandal, so we will proceed with the revert.",
					]);
					index = 2;
				} else {
					Morebits.status.error("Error", [
						"Latest revision was made by ",
						Morebits.htmlNode("strong", lastuser),
						", so it might have already been reverted, we will stop the revert.",
					]);
					return;
				}
			} else {
			...
			if (Twinkle.fluff.trustedBots.indexOf(params.user) !== -1) {
				switch (params.type) {
					case "vand":
						Morebits.status.info("Info", [
							"Vandalism revert was chosen on ",
							Morebits.htmlNode("strong", userNorm),
							". As this is a trusted bot, we assume you wanted to revert vandalism made by the previous user instead.",
						]);
						index = 2;
					...
					case "agf":
						Morebits.status.warn("Notice", [
							"Good faith revert was chosen on ",
							Morebits.htmlNode("strong", userNorm),
							". This is a trusted bot and thus AGF rollback will not proceed.",
						]);
						return;
					case "norm":
					/* falls through */
					default:
						var cont = confirm(
							"Normal revert was chosen, but the most recent edit was made by a trusted bot (" +
								userNorm +
								"). Do you want to revert the revision before instead?"
						);
						if (cont) {
							Morebits.status.info("Info", [
								"Normal revert was chosen on ",
								Morebits.htmlNode("strong", userNorm),
								". This is a trusted bot, and per confirmation, we'll revert the previous revision instead.",
							]);
							index = 2;
							params.user = revs[1].user;
							params.userHidden = !!revs[1].userhidden;
							...
							Morebits.status.warn("Notice", [
								"Normal revert was chosen on ",
								Morebits.htmlNode("strong", userNorm),
								". This is a trusted bot, but per confirmation, revert on selected revision will proceed.",
							]);
						}
						break;
				}
				...
				if (revs[i].user !== params.user) {
					...
					if (
						...
						if (!seen64) {
							new Morebits.status(
								"Note",
								"Treating consecutive IPv6 addresses in the same /64 as the same user"
							);
							seen64 = true;
						}
						continue;
			...
			if (!found) {
				statelem.error([
					"No previous revision found. Perhaps ",
					Morebits.htmlNode("strong", userNorm),
					" is the only contributor, or they have made more than " +
			...
			if (!count) {
				Morebits.status.error(
					"Error",
					"As it is not possible to revert zero revisions, we will stop this revert. It could be that the edit has already been reverted, but the revision ID was still the same."
				);
				return;
			}
				...
				if (
					!confirm(
						userNorm +
							...
							" edits in a row. Are you sure you want to revert them all?"
					)
				) {
			...
			switch (params.type) {
				case "agf":
					extra_summary = prompt(
						"An optional comment for the edit summary:                              ",
						""
					); // padded out to widen prompt in Firefox
					...
					summary = Twinkle.fluff.formatSummary(
						"Reverted [[WP:AGF|good faith]] edits by $USER",
				...
				case "vand":
					summary = Twinkle.fluff.formatSummary(
						"Reverted " +
							params.count +
							(params.count > 1 ? " edits" : " edit") +
							" by $USER to last revision by " +
							(params.gooduserHidden
								? Twinkle.fluff.hiddenName
								: params.gooduser),
						params.userHidden ? null : params.user
					);
					break;
			...
			if (
				...
				!confirm("Reverting page: are you sure?")
			) {
				statelem.error("Aborted by user.");
				return;
			}
			...
			var query = {
				...
				starttimestamp: loadtimestamp,
				minor:
					Twinkle.getPref("markRevertedPagesAsMinor").indexOf(params.type) !==
					-1
						? true
						: undefined,
				format: "json",
		...
		complete: function (apiobj) {
			...
			if (edit.captcha) {
				apiobj.statelem.error(
					"Could not rollback, because the wiki server wanted you to fill out a CAPTCHA."
				);
			} else if (edit.nochange) {
				apiobj.statelem.error(
					"Revision we are reverting to is identical to current revision, stopping revert."
				);
			} else {
				...
				if (params.notifyUser && !params.userHidden) {
					...
					Morebits.status.info("Info", [
						"Opening user talk page edit form for user ",
				...
				if (apiobj.params.reviewRevert) {
					...
					var wikipedia_api = new Morebits.wiki.api(
						"Automatically accepting your changes",
						query
					);
					wikipedia_api.post();
				}
	...
	Twinkle.fluff.formatSummary = function (
		builtInString,
		userName,
		customString
	) {
			...
			if (userName) {
				...
				if (resultLen + contribsLen <= 499) {
					...
					if (
						resultLen +
							contribsLen +
							unescape(encodeURIComponent(talkLink)).length <=
						499
					) {
						result = Morebits.string.safeReplace(
							result,
							"$USER",
