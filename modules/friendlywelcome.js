...
(function ($) {
	...
	Twinkle.welcome.normal = function () {
		...
		if (isDiff) {
			...
			var $oldDiffHasRedlinkedTalkPage = $oldDiffUsernameLine
				.find("span.mw-usertoollinks a.new:contains(talk)")
				.first();
			var $newDiffHasRedlinkedTalkPage = $newDiffUsernameLine
				.find("span.mw-usertoollinks a.new:contains(talk)")
				.first();

			var diffHasRedlinkedTalkPage =
			...
			if (diffHasRedlinkedTalkPage) {
				...
				if ($oldDiffHasRedlinkedTalkPage.length > 0) {
					...
					$oldDiffHasRedlinkedTalkPage[0].parentNode.parentNode.appendChild(
				...
				if ($newDiffHasRedlinkedTalkPage.length > 0) {
					...
					$newDiffHasRedlinkedTalkPage[0].parentNode.parentNode.appendChild(
						document.createTextNode(" ")
					);
					$newDiffHasRedlinkedTalkPage[0].parentNode.parentNode.appendChild(
						nWelcomeNode
	...
	Twinkle.welcome.welcomeUser = function welcomeUser() {
		...
		Morebits.wiki.actionCompleted.notice =
			"Welcoming complete, reloading talk page in a few seconds";
	...
	Twinkle.welcome.callback = function friendlywelcomeCallback(uid) {
		if (
			uid === mw.config.get("wgUserName") &&
			!confirm("Are you really sure you want to welcome yourself?...")
		) {
			return;
		}
	...
	Twinkle.welcome.populateWelcomeList = function (e) {
		...
		if (
			(type === "standard" || type === "anonymous") &&
			Twinkle.getPref("customWelcomeList").length
		) {
				...
				list: $.map(templates, function (properties, template) {
					return {
						...
						label:
							...
							(properties.linkedArticle ? "\u00A0*" : ""), // U+00A0 NO-BREAK SPACE
						tooltip: properties.tooltip, // may be undefined
					};
				}),
				event: function (ev) {
					ev.target.form.article.disabled =
						!templates[ev.target.value].linkedArticle;
				},
			});
		});
		...
		e.target.form.article.disabled = vals[firstRadio.value]
			? !vals[firstRadio.value].linkedArticle
			: true;
	};
	...
	Twinkle.welcome.templates = {
		standard: {
			"General welcome templates": {
				...
				"welcome-cookie": {
					description:
						"a welcome message with some helpful links and a plate of cookies",
					syntax: "{{subst:welcome cookie}} ~~~~",
				},
				welcoming: {
					description:
						"welcome message with tutorial links and basic editing tips",
			...
			"Specific welcome templates": {
				...
				"welcome student": {
					description:
						"welcome for students editing as part of an educational class project",
					syntax: "$HEADER$ {{subst:welcome student|$USERNAME$}} ~~~~",
				},
				"welcome teacher": {
					description:
						"welcome for course instructors involved in an educational class project",
					syntax: "$HEADER$ {{subst:welcome teacher|$USERNAME$}} ~~~~",
				},
				"welcome non-latin": {
					description:
						"welcome for users with a username containing non-Latin characters",
			...
			"Problem user welcome templates": {
				"first article": {
					description:
						"for someone whose first article did not meet page creation guidelines",
					linkedArticle: true,
					syntax: "{{subst:first article|$ARTICLE$|$USERNAME$}}",
				},
				"welcome-COI": {
					description:
						"for someone who has edited in areas where they may have a conflict of interest",
					linkedArticle: true,
				...
				"welcome-copyright": {
					description:
						"for someone who has been adding copyright violations to articles",
					linkedArticle: true,
					syntax: "{{subst:welcome-copyright|$ARTICLE$|$USERNAME$}} ~~~~",
				},
				"welcome-delete": {
					description:
						"for someone who has been removing information from articles",
					linkedArticle: true,
					syntax: "{{subst:welcome-delete|$ARTICLE$|$USERNAME$}} ~~~~",
				},
				"welcome-image": {
					description:
						"welcome with additional information about images (policy and procedure)",
					linkedArticle: true,
					syntax: "{{subst:welcome-image|$USERNAME$|art=$ARTICLE$}}",
				},
				welcomelaws: {
					description:
						"welcome with information about copyrights, NPOV, the sandbox, and vandalism",
					syntax: "{{subst:welcomelaws|$USERNAME$}} ~~~~",
				},
				welcomenpov: {
					description:
						"for someone whose initial efforts do not adhere to the neutral point of view policy",
					linkedArticle: true,
				...
				welcomevandal: {
					description:
						"for someone whose initial efforts appear to be vandalism",
					linkedArticle: true,
					syntax: "{{subst:welcomevandal|$ARTICLE$|$USERNAME$}}",
				},
				welcomespam: {
					description:
						"welcome with additional discussion of anti-spamming policies",
					linkedArticle: true,
		...
		anonymous: {
			"Anonymous user welcome templates": {
				...
				thanks: {
					description:
						"for anonymous users; short; encourages creating an account",
					linkedArticle: true,
				...
				"welcome-anon-unconstructive": {
					description:
						"for anonymous users who have vandalized or made unhelpful edits",
					linkedArticle: true,
					syntax: "{{subst:welcome-anon-unconstructive|$ARTICLE$|$USERNAME$}}",
				},
				"welcome-anon-constructive": {
					description:
						"for anonymous users who fight vandalism or edit constructively",
					linkedArticle: true,
					syntax: "{{subst:welcome-anon-constructive|art=$ARTICLE$}}",
				},
				"welcome-anon-delete": {
					description:
						"for anonymous users who have removed content from pages",
					linkedArticle: true,
		...
		wikiProject: {
			"WikiProject-specific welcome templates": {
				"TWA invite": {
					description:
						"invite the user to The Wikipedia Adventure (not a welcome template)",
					syntax: "{{subst:WP:TWA/InviteTW|signature=~~~~}}",
				},
				"welcome-anatomy": {
					description:
						"welcome for users with an apparent interest in anatomy topics",
					syntax: "{{subst:welcome-anatomy}} ~~~~",
				},
				"welcome-athletics": {
					description:
						"welcome for users with an apparent interest in athletics (track and field) topics",
					syntax: "{{subst:welcome-athletics}}",
				},
				"welcome-au": {
					description:
						"welcome for users with an apparent interest in Australia topics",
					syntax: "{{subst:welcome-au}} ~~~~",
				},
				"welcome-bd": {
					description:
						"welcome for users with an apparent interest in Bangladesh topics",
					linkedArticle: true,
					syntax: "{{subst:welcome-bd|$USERNAME$||$EXTRA$|art=$ARTICLE$}} ~~~~",
				},
				"welcome-bio": {
					description:
						"welcome for users with an apparent interest in biographical topics",
					syntax: "{{subst:welcome-bio}} ~~~~",
				},
				"welcome-cal": {
					description:
						"welcome for users with an apparent interest in California topics",
					syntax: "{{subst:welcome-cal}} ~~~~",
				},
				"welcome-conserv": {
					description:
						"welcome for users with an apparent interest in conservatism topics",
					syntax: "{{subst:welcome-conserv}}",
				},
				"welcome-cycling": {
					description:
						"welcome for users with an apparent interest in cycling topics",
					syntax: "{{subst:welcome-cycling}} ~~~~",
				},
				"welcome-dbz": {
					description:
						"welcome for users with an apparent interest in Dragon Ball topics",
					syntax: "{{subst:welcome-dbz|$EXTRA$|sig=~~~~}}",
				},
				"welcome-et": {
					description:
						"welcome for users with an apparent interest in Estonia topics",
					syntax: "{{subst:welcome-et}}",
				},
				"welcome-de": {
					description:
						"welcome for users with an apparent interest in Germany topics",
					syntax: "{{subst:welcome-de}} ~~~~",
				},
				"welcome-in": {
					description:
						"welcome for users with an apparent interest in India topics",
					linkedArticle: true,
					syntax: "{{subst:welcome-in|$USERNAME$|art=$ARTICLE$}} ~~~~",
				},
				"welcome-math": {
					description:
						"welcome for users with an apparent interest in mathematical topics",
					linkedArticle: true,
					syntax: "{{subst:welcome-math|$USERNAME$|art=$ARTICLE$}} ~~~~",
				},
				"welcome-med": {
					description:
						"welcome for users with an apparent interest in medicine topics",
					linkedArticle: true,
					syntax: "{{subst:welcome-med|$USERNAME$|art=$ARTICLE$}} ~~~~",
				},
				"welcome-no": {
					description:
						"welcome for users with an apparent interest in Norway topics",
					syntax: "{{subst:welcome-no}} ~~~~",
				},
				"welcome-pk": {
					description:
						"welcome for users with an apparent interest in Pakistan topics",
					linkedArticle: true,
					syntax: "{{subst:welcome-pk|$USERNAME$|art=$ARTICLE$}} ~~~~",
				},
				"welcome-phys": {
					description:
						"welcome for users with an apparent interest in physics topics",
					linkedArticle: true,
					syntax: "{{subst:welcome-phys|$USERNAME$|art=$ARTICLE$}} ~~~~",
				},
				"welcome-pl": {
					description:
						"welcome for users with an apparent interest in Poland topics",
					syntax: "{{subst:welcome-pl}} ~~~~",
				},
				"welcome-rugbyunion": {
					description:
						"welcome for users with an apparent interest in rugby union topics",
					syntax: "{{subst:welcome-rugbyunion}} ~~~~",
				},
				"welcome-ru": {
					description:
						"welcome for users with an apparent interest in Russia topics",
					syntax: "{{subst:welcome-ru}} ~~~~",
				},
				"welcome-starwars": {
					description:
						"welcome for users with an apparent interest in Star Wars topics",
					syntax: "{{subst:welcome-starwars}} ~~~~",
				},
				"welcome-ch": {
					description:
						"welcome for users with an apparent interest in Switzerland topics",
					linkedArticle: true,
					syntax: "{{subst:welcome-ch|$USERNAME$|art=$ARTICLE$}} ~~~~",
				},
				"welcome-uk": {
					description:
						"welcome for users with an apparent interest in Ukraine topics",
					syntax: "{{subst:welcome-uk}} ~~~~",
				},
				"welcome-roads": {
					description:
						"welcome for users with an apparent interest in roads and highways topics",
					syntax: "{{subst:welcome-roads}}",
				},
				"welcome-videogames": {
					description:
						"welcome for users with an apparent interest in video game topics",
					syntax: "{{subst:welcome-videogames}}",
				},
				"WikiProject Women in Red invite": {
					description:
						"welcome for users with an intrest in writing about women",
		...
		nonEnglish: {
			"Non-English welcome templates": {
				welcomeen: {
					description:
						"welcome for users whose first language is not listed here",
					syntax: "{{subst:welcomeen}}",
				},
				"welcomeen-ar": {
					description:
						"welcome for users whose first language appears to be Arabic",
					syntax: "{{subst:welcomeen-ar}}",
				},
				"welcomeen-sq": {
					description:
						"welcome for users whose first language appears to be Albanian",
					syntax: "{{subst:welcomeen-sq}}",
				},
				"welcomeen-zh": {
					description:
						"welcome for users whose first language appears to be Chinese",
					syntax: "{{subst:welcomeen-zh}}",
				},
				"welcomeen-nl": {
					description:
						"welcome for users whose first language appears to be Dutch",
					syntax: "{{subst:welcomeen-nl}}",
				},
				"welcomeen-fi": {
					description:
						"welcome for users whose first language appears to be Finnish",
					syntax: "{{subst:welcomeen-fi}}",
				},
				"welcomeen-fr": {
					description:
						"welcome for users whose first language appears to be French",
					syntax: "{{subst:welcomeen-fr}}",
				},
				"welcomeen-de": {
					description:
						"welcome for users whose first language appears to be German",
					syntax: "{{subst:welcomeen-de}}",
				},
				"welcomeen-he": {
					description:
						"welcome for users whose first language appears to be Hebrew",
					syntax: "{{subst:welcomeen-he}}",
				},
				"welcomeen-ja": {
					description:
						"welcome for users whose first language appears to be Japanese",
					syntax: "{{subst:welcomeen-ja}}",
				},
				"welcomeen-ko": {
					description:
						"welcome for users whose first language appears to be Korean",
					syntax: "{{subst:welcomeen-ko}}",
				},
				"welcomeen-ml": {
					description:
						"welcome for users whose first language appears to be Malayalam",
					syntax: "{{subst:welcomeen-ml}}",
				},
				"welcomeen-mr": {
					description:
						"welcome for users whose first language appears to be Marathi",
					syntax: "{{subst:welcomeen-mr}}",
				},
				"welcomeen-or": {
					description:
						"welcome for users whose first language appears to be Oriya (Odia)",
					syntax: "{{subst:welcomeen-or}}",
				},
				"welcomeen-pt": {
					description:
						"welcome for users whose first language appears to be Portuguese",
					syntax: "{{subst:welcomeen-pt}}",
				},
				"welcomeen-ro": {
					description:
						"welcome for users whose first language appears to be Romanian",
					syntax: "{{subst:welcomeen-ro}}",
				},
				"welcomeen-ru": {
					description:
						"welcome for users whose first language appears to be Russian",
					syntax: "{{subst:welcomeen-ru}}",
				},
				"welcomeen-es": {
					description:
						"welcome for users whose first language appears to be Spanish",
					syntax: "{{subst:welcomeen-es}}",
				},
				"welcomeen-sv": {
					description:
						"welcome for users whose first language appears to be Swedish",
					syntax: "{{subst:welcomeen-sv}}",
				},
				"welcomeen-uk": {
					description:
						"welcome for users whose first language appears to be Ukrainian",
					syntax: "{{subst:welcomeen-uk}}",
				},
				"welcomeen-id": {
					description:
						"welcome for users whose first language appears to be Indonesian",
	...
	Twinkle.welcome.getTemplateWikitext = function (type, template, article) {
		...
		if (properties) {
			return properties.syntax
				.replace(
					"$USERNAME$",
					Twinkle.getPref("insertUsername") ? mw.config.get("wgUserName") : ""
				)
	...
	Twinkle.welcome.callbacks = {
		preview: function (form) {
			...
			previewer.beginRender(
				Twinkle.welcome.getTemplateWikitext(
					input.type,
					input.template,
					input.article
				),
		...
		main: function (pageobj) {
			...
			if (pageobj.exists() && params.mode === "auto") {
				Morebits.status.info(
					"Warning",
					"User talk page not empty; aborting automatic welcome"
				);
	...
	Twinkle.welcome.callback.evaluate = function friendlywelcomeCallbackEvaluate(
		e
	) {
		var form = e.target;
