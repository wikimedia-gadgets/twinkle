	...
	'Cleanup and maintenance tags': {
		...
		'Potentially unwanted content': [
			...
			}, // has a subgroup with text input
			{
				...
				subgroup: mw.config.get('wgNamespaceNumber') === 0 ? [
					{
						name: 'llmReason',
						parameter: 'reason',
						type: 'input',
						label: 'Explanation (will be added to the tag):',
						tooltip: 'Optional, but recommended to fill one or the other. Leave blank if not wanted.'
					},
					{
						name: 'llmReasonTalk',
						type: 'textarea',
						label: 'Detailed explanation (will be posted on this article\'s talk page):',
						tooltip: 'Optional, but recommended to fill one or the other. Leave blank if not wanted.'
					}
				] :
				{
					...
					type: 'input',
					label: 'Explanation (will be added to the tag):',
					tooltip: 'Optional, but recommended. Leave blank if not wanted.'
				}
			},
...
Twinkle.tag.callbacks = {
	article: function articleCallback(pageobj) {
		...
		const postRemoval = function() {
			...
			pageobj.save(() => {
				// COI, LLM tagging: Start the discussion on the talk page (mainspace only)
				if (params.coiReason) {
				...
				if (params.llmReasonTalk) {
					const llmTalkPage = new Morebits.wiki.Page('Talk:' + Morebits.pageNameNorm, 'Starting discussion on talk page');
					llmTalkPage.setNewSectionText(params.llmReasonTalk + ' ~~~~');
					llmTalkPage.setNewSectionTitle('AI-generated tag (' + new Morebits.Date(pageobj.getLoadTime()).format('MMMM Y', 'utc') + ')');
					llmTalkPage.setChangeTags(Twinkle.changeTags);
					llmTalkPage.setCreateOption('recreate');
					llmTalkPage.newSection();
				}
