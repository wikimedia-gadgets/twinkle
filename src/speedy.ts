const {arr_includes, obj_entries, obj_values} = Twinkle.shims;

class Speedy extends TwinkleModule {
	...
	constructor() {
		super();
		this.portletName = 'CSD';
		this.portletId = 'twinkle-csd';
		this.portletTooltip = Morebits.userIsSysop ? 'Delete page according to WP:CSD' : 'Request speedy deletion according to WP:CSD';
		this.addMenu();
	}
	...
	makeWindow = () => {
		...
		this.dialog.setScriptName('Twinkle');
		...
		var form = new Morebits.quickForm((e) => this.evaluate(e), Twinkle.getPref('speedySelectionStyle') === 'radioClick' ? 'change' : null);
		this.form = form;
	...
	appendCriteriaLists(work_area: Morebits.quickForm.element) {
		...
		var inputType = (this.mode.isMultiple ? 'checkbox' : 'radio') as 'radio' | 'checkbox';
	...
	tagPage() {
		...
		pageobj.load((pageobj) => {
			...
			if (params.normalizeds[0] === 'db') {
				...
				var criteriaText = params.normalizeds.map((norm) => {
	...
	evaluateSysop() {
		...
		let form = this.result, values = this.params.csd, normalizeds = this.params.normalizeds;
	...
	sysopMain() {
		...
		if (!params.normalizeds.length && params.normalizeds[0] === 'db') {
			...
			this.parseWikitext(code, (reason) => {
	...
	deleteRedirects() {
		...
		if (params.deleteRedirects) {
			...
			var wikipedia_api = new Morebits.wiki.api('getting list of redirects...', query,
				this.deleteRedirectsMain, new Morebits.status('Deleting redirects'));
			wikipedia_api.post();
		}
...
interface criterion extends quickFormElementData {
...
interface criteriaSubgroup extends quickFormElementData {
	parameter?: string
	utparam?: string
	log?: string | null
}

Twinkle.addInitCallback(() => { new Speedy(); }, 'CSD');
